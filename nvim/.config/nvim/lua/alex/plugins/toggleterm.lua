...
M.terminal = {
    ...
    -- M.terminal.execs = {...} to overwrite
    -- M.terminal.execs[#alex.plugins.terminal.execs+1] = {"gdb", "tg", "GNU Debugger"}
...
M.init = function ()
    for i, exec in pairs(M.terminal.execs) do
        local direction = exec[4] or M.terminal.direction

        local opts = {
            cmd = exec[1] or M.terminal.shell or vim.o.shell,
            keymap = exec[2],
            label = exec[3],
